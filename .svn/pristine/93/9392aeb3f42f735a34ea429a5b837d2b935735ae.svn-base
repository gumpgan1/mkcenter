...
//判断用户信息，然后将其他函数加在里面，一起放在window.onload
function login(){
	$.ajax({
    	type:'POST',
    	url:'../checkLogin.do',
...
//    		$("#u434_input").append('<option value="out">总料位反馈值</option>');
	...
	// 默认配置导入自定义配置
		$.ajax({
	    	type:'GET',
	    	url:'../board/getConfig.do?line_id='+id,
	    	timeout:2000,
	    	async:false,//同步，保证有返回值(配合return使用)，为确保用户体验，使用异步+赋值的方法
	    	success:function(data){
	    		var json =  eval("("+data+")");
	    		for(var key in json.list){
	    			if(json.list[key].CODE=='init'){
...
//	    		$("#u434_input").append('<option value="out">总料位反馈值</option>');
	...
	$.ajax({
		type:'GET',
    	url:'../limo/getDefaultConfig.do?line_id='+id,
    	timeout:2000,
    	async:false,//同步，保证有返回值(配合return使用)，为确保用户体验，使用异步+赋值的方法
    	success:function(data){
    		var json =  eval("("+data+")");
...
function initchart(){
	...
	pars = init_t;//图标类型
	for(var i = 0;i<=5;i++){		
		var s = $("#u434_input option[value='"+stas[i]+"]").text();//获得工位点对应中文名
		console.log(s);  //工位点id		
		$("#"+(i+1)).html('<b>'+s+'</b>');//将工位名称显示在小面板p属性上
		var tg = i+423;       
		$("#u"+tg).attr("title",s);//将工位名作为attr显示在div上，当鼠标悬浮时有显示
	}
...
function save(){
	var num = $("#num").text();//当前改的是第几个点
	if(num==''||num==null){
		return;
	}
	var bef = stas[num];//bef是原本的的工位点
	var sta1 = $("#u434_input").find("option:selected").val();//当前改的工位名
	var ser = $("#u439_input").find("option:selected").val();//当前改的图表类型

	init[num]=sta1;//更新位置的对应工位点
	init_t[num]=ser;//更新位置的对应图表类型
	var tg = parseInt(num)+422; //tg是container+tag
	//销毁图
	...
	switch(ser){
	case '1':
		initSpline("container"+num,sta1);
		break;
	case '2':
		initSpeed("container"+num,sta1);
		break;
	case '3':
		initVA("container"+num,sta1);
		break;
	  default:
	  	break;
	}
	...
	$.ajax({
		type:'POST',
    	url:'../board/updateConfig.do?line_id=' + id,
    	data:formData, 
		contentType: false, 
	    processData: false,
	    beforeSend:function(){
